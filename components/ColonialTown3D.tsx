...
function Scene({
  ...
  const isBlocked = useCallback((x: number, z: number) => {
    ...
    return unlockedProps.some((prop) => (
      prop.type === "Cottage" &&
      Math.abs(x - prop.position[0]) < 4.5 &&
      Math.abs(z - prop.position[2]) < 4.5
    ));
  }, [unlockedProps]);
  ...
  useFrame((state, delta) => {
    ...
    if (f !== 0 || r !== 0) {
      ...
      if (!blockedByBuilding && !outOfBounds) {
        ...
        slideX.x = next.x;
        if (!isBlocked(slideX.x, slideX.z) && slideX.x > -85 && slideX.x < 85) {
          cam.position.x = slideX.x;
        }
        const slideZ = cam.position.clone();
        slideZ.z = next.z;
        if (!isBlocked(slideZ.x, slideZ.z) && slideZ.z > -95 && slideZ.z < 85) {
