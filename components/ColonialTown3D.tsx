...
function playDoorSound() {
  try {
    ...
    setTimeout(() => {
      const c2=new (window.AudioContext||(window as any).webkitAudioContext)(),o2=c2.createOscillator(),g2=c2.createGain();
      o2.type="sine"; o2.frequency.value=528;
      g2.gain.setValueAtTime(0.25,c2.currentTime); g2.gain.exponentialRampToValueAtTime(0.001,c2.currentTime+1.4);
...
export default function ColonialTown3D() {
  ...
  const showStart  = !locked && (!isMobile || !avatarPicked);
  const showMobile = isMobile && avatarPicked && locked;
  ...
  return (
    <div className="relative w-full h-full">
      ...
      {/* Avatar picker + start screen */}
      {!locked && (
        <div className="absolute inset-0 z-20 flex items-center justify-center bg-black/70 backdrop-blur-sm"
             onClick={avatarPicked ? () => document.querySelector("canvas")?.click() : undefined}>
          <div className="text-center px-8 py-7 rounded-3xl mx-4 max-w-sm w-full"
               style={{ background:"rgba(8,4,2,0.97)", border:"1px solid rgba(201,168,76,0.5)",
                        fontFamily:"EB Garamond, serif", boxShadow:"0 0 60px rgba(201,168,76,0.12)" }}>
            ...
            <div className="grid grid-cols-3 gap-2 mb-5">
              {AVATARS.map((a, i) => (
                <button key={i} onClick={(e) => { e.stopPropagation(); pickAvatar(i); }}
                  ...
                  <p className="font-cinzel text-center leading-tight"
                     style={{ fontSize:9, color: avatarIdx===i ? "#c9a84c" : "#6b4423" }}>
                    {a.title}
                  </p>
                </button>
              ))}
            ...
            <button
              onClick={() => { setAvatarPicked(true); document.querySelector("canvas")?.click(); }}
              className="w-full rounded-2xl py-3 font-cinzel font-bold text-base transition-all"
              style={{ background:"#c9a84c", color:"#1a0f0a", opacity: avatarPicked ? 1 : 0.5 }}>
              Enter the Colony
            </button>
            {!isMobile && (
      ...
      {(locked || (isMobile && avatarPicked)) && Object.keys(others).length > 0 && (
        <div className="absolute top-3 right-3 z-20 flex items-center gap-2 px-3 py-1.5 rounded-xl"
      ...
      {locked && !isMobile && !entering && (
        <p className="absolute top-3 left-1/2 -translate-x-1/2 z-20 pointer-events-none text-xs font-cinzel"
           style={{ color:"rgba(201,168,76,0.35)" }}>ESC to release mouse</p>
      )}

      {showMobile && <MobileControls moveRef={moveRef} yawRef={mobileYaw} />}

      {entering && (
        <EntryTransition building={entering} onDone={() => { router.push(entering.href); setEntering(null); }} />
      )}
    </div>
  );
}
  ...
  return (
    <>
      ...
      {Object.entries(others).map(([id, p]) => (
        <OtherPlayer key={id} x={p.x} z={p.z} yaw={p.yaw}
                     avatarIdx={p.avatarIdx ?? 0} name={p.username ?? "Colonist"} />
      ))}
  ...
  return (
    <group ref={group} position={[x, 0, z]}>
      ...
      <mesh position={[0, 1.1, 0]}>
        ...
        <meshStandardMaterial color={av.color} roughness={0.85} envMapIntensity={0.3} />
      </mesh>
      {/* Coat tails */}
      ...
      {/* Tricorn folds */}
      {[0, 2.09, -2.09].map((ry, i) => (
        <mesh key={i} position={[Math.sin(ry)*0.22, 1.89, Math.cos(ry)*0.22]} rotation={[0,ry,0.35]}>
...
function Ground() {
  return (
    <>
      ...
      {Array.from({length:30},(_,row)=>Array.from({length:8},(__,col)=>{
...
function Street() {
  return (
    <>
      ...
      {[-3.8,3.8].map((sx,si)=>Array.from({length:16},(_,i)=>(
    ...
    <group position={[x,0,z]}>
      ...
      {[[-w*0.28,-d*0.18],[w*0.3,d*0.15]].map(([cx,cz],ci)=>(
        <group key={ci} position={[cx,h+0.44,cz]}>
          <mesh castShadow><boxGeometry args={[0.65,large?3.8:2.8,0.65]} /><meshStandardMaterial color={roof} roughness={0.97} /></mesh>
          <mesh position={[0,large?2:1.5,0]}><boxGeometry args={[0.85,0.18,0.85]} /><meshStandardMaterial color="#0e0c0a" roughness={1} /></mesh>
      ...
      {(large?[-w*0.32,0,w*0.32]:[-w*0.28,w*0.28]).map((wx,wi)=>(
        <group key={wi} position={[wx,h*0.6+0.44,fz+fDir*0.08]}>
          ...
          <mesh position={[0,0,0.06]}><boxGeometry args={[0.82,1.15,0.04]} /><meshStandardMaterial color={win} emissive={win} emissiveIntensity={1.8} transparent opacity={0.75} roughness={0.04} metalness={0.6} envMapIntensity={1.5} /></mesh>
      ...
      {pillars && [-w*0.32,-w*0.12,w*0.12,w*0.32].map((px,pi)=>(
        <mesh key={pi} position={[px,h*0.3+0.44,fz+fDir*0.5]} castShadow>
          <cylinderGeometry args={[0.18,0.22,h*0.6,10]} /><meshStandardMaterial color="#221808" roughness={0.88} envMapIntensity={0.3} />
        </mesh>
      ))}
      {pillars && <mesh position={[0,h*0.6+0.44,fz+fDir*0.45]}><boxGeometry args={[w*0.85,0.3,0.5]} /><meshStandardMaterial color="#1e1408" roughness={0.9} /></mesh>}
      {[[-w/2,0],[w/2,0]].map(([qx],qi)=>Array.from({length:Math.floor(h/1.4)},(_,ri)=>(
        <mesh key={`${qi}-${ri}`} position={[qx+(qx>0?.05:-.05),ri*1.4+0.9,0]}>
          <boxGeometry args={[0.14,0.55,d+0.14]} /><meshStandardMaterial color={`hsl(30,${25+ri*2}%,${12+ri}%)`} roughness={0.93} />
        </mesh>
      )))}
      <pointLight position={[0,h*0.6,fz+fDir*1.8]} intensity={3.5} distance={10} color={win} decay={2} castShadow={false} />
      <pointLight position={[0,h*0.6,0]} intensity={0.8} distance={6} color={win} decay={2} />
      <group position={[0,h+0.44+0.8,fz+fDir*0.5]}>
        <mesh><boxGeometry args={[w*0.5,0.5,0.08]} /><meshStandardMaterial color="#1e1208" roughness={0.9} /></mesh>
        <mesh position={[0,0,0.05]}><boxGeometry args={[w*0.46,0.42,0.04]} /><meshStandardMaterial color="#2a1a08" roughness={0.88} /></mesh>
      </group>
      <Html position={[0,h+8,0]} center distanceFactor={10} style={{pointerEvents:"none",userSelect:"none"}}>
        <div style={{ textAlign:"center", fontFamily:"EB Garamond, serif", color:"#ffd700",
...
function LanternPost({ position }: { position:[number,number,number] }) {
  ...
  return (
    <group position={[x,0,z]}>
      <mesh position={[0,0.15,0]} receiveShadow><cylinderGeometry args={[0.14,0.18,0.3,8]} /><meshStandardMaterial color="#1e1208" roughness={0.95} /></mesh>
      <mesh position={[0,2.8,0]} castShadow><cylinderGeometry args={[0.055,0.07,5.3,8]} /><meshStandardMaterial color="#2a1808" roughness={0.88} metalness={0.15} envMapIntensity={0.5} /></mesh>
      <mesh position={[0.35,5.2,0]} rotation={[0,0,-0.4]} castShadow><cylinderGeometry args={[0.035,0.035,0.9,6]} /><meshStandardMaterial color="#2a1808" roughness={0.88} metalness={0.15} /></mesh>
      <group position={[0.62,5.35,0]}>
        <mesh><boxGeometry args={[0.32,0.44,0.32]} /><meshStandardMaterial color="#c9a84c" emissive="#c9a84c" emissiveIntensity={0.9} transparent opacity={0.82} roughness={0.05} metalness={0.3} /></mesh>
        <mesh position={[0,0.32,0]}><coneGeometry args={[0.28,0.22,4]} /><meshStandardMaterial color="#1e1208" roughness={0.9} /></mesh>
...
function Fountain() {
  return (
    <group position={[0,0,0]}>
      <mesh position={[0,0.25,0]} receiveShadow castShadow><cylinderGeometry args={[2.6,3,0.5,16]} /><meshStandardMaterial color="#1a1208" roughness={0.96} /></mesh>
      <mesh position={[0,0.65,0]} receiveShadow><cylinderGeometry args={[2,2.4,0.3,16]} /><meshStandardMaterial color="#141008" roughness={0.97} /></mesh>
      <mesh position={[0,0.82,0]} rotation={[-Math.PI/2,0,0]}><circleGeometry args={[1.8,24]} /><meshStandardMaterial color="#1a4060" emissive="#1a4060" emissiveIntensity={0.4} transparent opacity={0.8} roughness={0.05} metalness={0.6} /></mesh>
      <mesh position={[0,1.5,0]} castShadow><cylinderGeometry args={[0.16,0.2,1.8,10]} /><meshStandardMaterial color="#1e1208" roughness={0.92} /></mesh>
      <mesh position={[0,2.55,0]}><sphereGeometry args={[0.3,14,14]} /><meshStandardMaterial color="#c9a84c" emissive="#c9a84c" emissiveIntensity={0.5} roughness={0.25} metalness={0.7} envMapIntensity={1.2} /></mesh>
    ...
    <>
      {positions.map(([x,,z,s],i)=>(
        <group key={i} position={[x,0,z]} scale={s}>
          <mesh position={[0,1.5,0]} castShadow><cylinderGeometry args={[0.18,0.28,3,7]} /><meshStandardMaterial color="#1a0e06" roughness={0.97} /></mesh>
          {[0,1,2].map(li=>(
            <mesh key={li} position={[0,3.5+li*1.2,0]} castShadow>
              <coneGeometry args={[1.6-li*0.35,2.2,7]} /><meshStandardMaterial color={`hsl(130,${25+li*5}%,${8+li*2}%)`} roughness={0.96} envMapIntensity={0.2} />
            </mesh>
          ))}
        </group>
      ))}
...
/* ─── Mobile controls ─────────────────────────────────────────── */
function MobileControls({ moveRef, yawRef }: {
  moveRef: React.MutableRefObject<{f:number;r:number}>;
  yawRef:  React.MutableRefObject<number>;
}) {
  const lStick=useRef<HTMLDivElement>(null), lTouch=useRef<number|null>(null);
  const lOrigin=useRef({x:0,y:0}), rTouch=useRef<number|null>(null), rLast=useRef({x:0,y:0});
  ...
  function onTouchMove(e:React.TouchEvent){
    for(const t of Array.from(e.changedTouches)){
      if(t.identifier===lTouch.current){
        const dx=(t.clientX-lOrigin.current.x)/45,dy=(t.clientY-lOrigin.current.y)/45;
  ...
  return (
    <div className="fixed inset-0 z-20" style={{touchAction:"none"}}
  ...
  return (
    <>
      ...
      <div className="fixed inset-0 z-50 flex flex-col items-center justify-center"
        ...
        <div style={{fontSize:76,marginBottom:20,animation:"entry-text-rise .5s .25s ease-out both,door-glow-pulse 1.2s .6s ease-in-out infinite"}}>{building.icon}</div>
