...
function playDoorSound() {
  try {
    ...
    setTimeout(() => {
      ...
      o2.start(); o2.stop(c2.currentTime + 1.4);
    }, 500);
...
export default function ColonialTown3D() {
  ...
  return (
    <div className="relative w-full h-full">
      ...
      {!locked && !isMobile && (
        <div
          className="absolute inset-0 z-20 flex items-center justify-center bg-black/65 backdrop-blur-sm cursor-pointer"
        ...
        >
          ...
          >
            ...
            <p className="text-base mb-1" style={{ color: "#9a7d5a" }}>Click anywhere to enter</p>
            <p className="text-sm mt-3 italic" style={{ color: "#5a3d22" }}>
      ...
      {!locked && isMobile && (
        ...
        >
          <div
            className="text-center px-10 py-8 rounded-3xl mx-6"
            style={{
              background: "rgba(8,4,2,0.95)",
              border: "1px solid rgba(201,168,76,0.55)",
              fontFamily: "EB Garamond, serif",
          ...
          >
            <div className="text-6xl mb-4">🏛</div>
            <h2 className="font-cinzel text-3xl font-bold mb-3" style={{ color: "#c9a84c" }}>
              Walk the Colony
            </h2>
            <p className="text-sm mb-4" style={{ color: "#9a7d5a" }}>
              Left thumb: move · Right drag: look
            </p>
            <div
              className="rounded-2xl px-8 py-3 font-cinzel text-base font-bold inline-block"
      ...
      {locked && !isMobile && !entering && (
        <p
          className="absolute top-3 left-1/2 -translate-x-1/2 z-20 pointer-events-none text-xs font-cinzel"
...
/* ─── Scene ───────────────────────────────────────────────────── */
function Scene({ moveRef, mobileYaw, isMobile, onNear, onLock }: {
  ...
  useFrame((state, delta) => {
    ...
    let found: typeof BUILDINGS[number] | null = null;
    let best = Infinity;
  ...
  return (
    <>
      ...
      <fog attach="fog" args={["#1a0f06", 28, 140]} />

      <ambientLight intensity={0.6} color="#ffe0b0" />
      <directionalLight
        position={[50, 80, -60]}
        intensity={2.2}
        color="#ffcc88"
        ...
        shadow-bias={-0.001}
      />
      <hemisphereLight args={["#3a2510", "#0a0604", 0.8]} />
    ...
    <>
      ...
      {Array.from({ length: 30 }, (_, row) =>
        Array.from({ length: 8 }, (__, col) => {
          const x = -13 + col * 3.8 + (row % 2) * 1.9;
          const z = -22 + row * 1.6;
          return (
            <mesh key={`${row}-${col}`} rotation={[-Math.PI / 2, 0, Math.random() * 0.3]}
                  position={[x, 0.005, z]} receiveShadow>
              <planeGeometry args={[1.6 + Math.random() * 0.3, 1.1 + Math.random() * 0.2]} />
              <meshStandardMaterial
                color={`hsl(30,${20 + Math.random() * 10}%,${7 + Math.random() * 4}%)`}
                roughness={0.96}
              />
            </mesh>
          );
    ...
    <group position={[x, 0, z]}>
      ...
      {(large ? [-w * 0.32, 0, w * 0.32] : [-w * 0.28, w * 0.28]).map((wx, wi) => (
        <group key={wi} position={[wx, h * 0.6 + 0.44, fz + fDir * 0.08]}>
          ...
          <mesh position={[0, 0, 0.06]}>
            ...
            <meshStandardMaterial
              color={win} emissive={win} emissiveIntensity={1.8}
              transparent opacity={0.75} roughness={0.04} metalness={0.6} envMapIntensity={1.5}
            />
          </mesh>
      ...
      {(large ? [-w * 0.3, w * 0.3] : [0]).map((wx, wi) => (
        <group key={wi} position={[wx, h * 0.82 + 0.44, fz + fDir * 0.08]}>
          ...
          <mesh position={[0, 0, 0.06]}>
            ...
            <meshStandardMaterial
              color={win} emissive={win} emissiveIntensity={1.4}
              transparent opacity={0.7} roughness={0.04} metalness={0.5}
            />
          </mesh>
        </group>
      ))}
      ...
      <group position={[0, h * 0.22 + 0.44, fz + fDir * 0.08]}>
        ...
        {[-0.33, 0.33].map((dpx, di) => (
          <mesh key={di} position={[dpx, 0.1, 0.12]}>
        ...
        <mesh position={[0, h * 0.24, 0.1]}>
          ...
          <meshStandardMaterial color={win} emissive={win} emissiveIntensity={1.2}
                                 transparent opacity={0.7} roughness={0.08} metalness={0.4} />
        </mesh>
        {[0, 1].map(si => (
      ...
      {pillars && [-w * 0.32, -w * 0.12, w * 0.12, w * 0.32].map((px, pi) => (
        <mesh key={pi} position={[px, h * 0.3 + 0.44, fz + fDir * 0.5]} castShadow>
          ...
          <meshStandardMaterial color="#221808" roughness={0.88} envMapIntensity={0.3} />
        </mesh>
      ))}
      ...
      {/* Window lights */}
      <pointLight position={[0, h * 0.6, fz + fDir * 1.8]}
                  intensity={3.5} distance={10} color={win} decay={2} castShadow={false} />
      <pointLight position={[0, h * 0.6, 0]}
                  intensity={0.8} distance={6} color={win} decay={2} />
      ...
      <group position={[0, h + 0.44 + 0.8, fz + fDir * 0.5]}>
        ...
        <mesh position={[0, 0, 0.05]}>
          <boxGeometry args={[w * 0.46, 0.42, 0.04]} />
          <meshStandardMaterial color="#2a1a08" roughness={0.88} />
        </mesh>
      </group>
      ...
      >
        <div style={{
          ...
          fontFamily: "EB Garamond, serif",
          color: "#c9a84c",
          textShadow: "0 0 16px rgba(201,168,76,0.9), 0 2px 4px rgba(0,0,0,0.8)",
          whiteSpace: "pre-line",
        }}>
          <div style={{ fontSize: 26, lineHeight: 1 }}>{icon}</div>
          <div style={{ fontSize: 12, fontWeight: "bold", letterSpacing: "0.1em", marginTop: 3 }}>
            {label}
          </div>
        </div>
      </Html>
  ...
  return (
    <group position={[x, 0, z]}>
      ...
      <mesh position={[0, 2.8, 0]} castShadow>
        ...
        <meshStandardMaterial color="#2a1808" roughness={0.88} metalness={0.15} envMapIntensity={0.5} />
      </mesh>
      ...
      <group position={[0.62, 5.35, 0]}>
        <mesh>
          ...
          <meshStandardMaterial color="#c9a84c" emissive="#c9a84c" emissiveIntensity={0.9}
                                 transparent opacity={0.82} roughness={0.05} metalness={0.3} />
        </mesh>
  ...
  return (
    <group position={[0, 0, 0]}>
      ...
      <mesh position={[0, 0.82, 0]} rotation={[-Math.PI / 2, 0, 0]}>
        ...
        <meshStandardMaterial color="#1a4060" emissive="#1a4060" emissiveIntensity={0.4}
                               transparent opacity={0.8} roughness={0.05} metalness={0.6} />
      </mesh>
      ...
      <mesh position={[0, 2.55, 0]}>
        ...
        <meshStandardMaterial color="#c9a84c" emissive="#c9a84c" emissiveIntensity={0.5}
                               roughness={0.25} metalness={0.7} envMapIntensity={1.2} />
      </mesh>
      <Sparkles count={30} scale={[3, 2, 3]} position={[0, 1.2, 0]}
                size={2} speed={0.8} color="#93c5fd" opacity={0.6} />
...
function MobileControls({
  ...
  function onTouchMove(e: React.TouchEvent) {
    for (const t of Array.from(e.changedTouches)) {
      if (t.identifier === lTouch.current) {
        ...
        if (lStick.current)
          lStick.current.style.transform =
  ...
  return (
    ...
    >
      ...
      >
        <p style={{ fontSize: 9, color: "rgba(201,168,76,0.35)", textAlign: "center",
                    fontFamily: "EB Garamond, serif" }}>
          drag<br />to look
        </p>
      </div>
  ...
  return (
    <>
      <style>{`
        ...
        @keyframes entry-text-rise {
          from { opacity: 0; transform: translateY(14px); }
          to   { opacity: 1; transform: translateY(0);   }
        }
        @keyframes door-glow-pulse {
          0%,100% { text-shadow: 0 0 20px rgba(201,168,76,0.5); }
          50%     { text-shadow: 0 0 55px rgba(201,168,76,1), 0 0 90px rgba(201,168,76,0.35); }
        }
        @keyframes grow-bar {
      ...
      <div
        ...
        style={{
          background: "radial-gradient(ellipse at center, rgba(0,0,0,0.05) 0%, rgba(0,0,0,0.98) 100%)",
          animation: "entry-fade-in 0.55s ease-out forwards",
        }}
      >
        {/* Icon */}
        <div
          style={{
            fontSize: 76,
            marginBottom: 20,
            animation: "entry-text-rise 0.5s 0.25s ease-out both, door-glow-pulse 1.2s 0.6s ease-in-out infinite",
          }}
        >
          {building.icon}
        </div>
        ...
        <h2
          style={{
            ...
            whiteSpace: "pre-line",
            marginBottom: 14,
            animation: "entry-text-rise 0.5s 0.4s ease-out both",
          }}
        >
          {building.label}
        </h2>
        ...
        <p
          style={{
            ...
            animation: "entry-text-rise 0.5s 0.6s ease-out both",
          }}
        >
          Entering…
        </p>
        ...
        <div
          style={{
            ...
            borderRadius: 99,
            overflow: "hidden",
            animation: "entry-text-rise 0.4s 0.65s ease-out both",
          }}
        >
          <div
            style={{
              ...
              animation: "grow-bar 1.6s 0.2s linear forwards",
              transform: "scaleX(0)",
            }}
          />
        </div>
