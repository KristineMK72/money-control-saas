    ...
    return (
      <main className="min-h-screen bg-transparent p-4 md:p-6">
        <div className={`${shellClass} mx-auto max-w-5xl`}>
          <div className={cardClass}>Loading bills...</div>
        </div>
      </main>
    );
  }
      ...
      <div className={`${shellClass} mx-auto max-w-5xl space-y-5`}>
        <header className={cardClass}>
          ...
          <p className="mt-2 max-w-2xl text-sm font-semibold text-zinc-700">
            Keep bills organized without covering up the whole BenWorld background.
        ...
        >
          <p className="text-sm font-semibold text-zinc-700">
            Upload a bill screenshot or receipt image. AskBen will try to fill in
            the name, amount, and date for you.
          </p>
          ...
          <input
            ...
            capture="environment"
            disabled={scanning}
            onChange={(e) => void scanBillImage(e.target.files?.[0] || null)}
        ...
        <DropdownCard
          id="add"
          title="Add Bill"
          value={name || amount ? "Draft ready" : "Manual entry"}
          openPanel={openPanel}
          setOpenPanel={setOpenPanel}
        >
        ...
        <section className="space-y-3">
          {bills.length === 0 ? (
            <div className={cardClass}>No bills yet. Add your first bill above.</div>
          ) : (
            bills.map((bill) => {
              const paid = getMonthlyPaid(bill.id);
              const target = Number(bill.target || 0);
              const pct = target > 0 ? Math.min((paid / target) * 100, 100) : 0;
                ...
                >
                  <div className="space-y-4">
                    <div className="grid gap-3 md:grid-cols-3">
                      <MiniStat label="Target" value={money(target)} />
                      <MiniStat label="Paid" value={money(paid)} />
                      <MiniStat
                        label="Remaining"
                        value={money(Math.max(0, target - paid))}
                      />
                    </div>
                    ...
                    <div>
                      <div className="mb-2 flex justify-between text-xs font-bold text-zinc-700">
                        <span>
                          {money(paid)} / {money(target)} paid this month
                        </span>
                    ...
                    <div className="text-xs font-semibold text-zinc-600">
                      {bill.category && <span>Category: {bill.category}</span>}
