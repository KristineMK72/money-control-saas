...
function getBillMonthlyAmount(bill: Bill): number {
...
function classifyBill(bill: Bill): BillKind {
  const hasCredit = bill.balance != null || bill.min_payment != null || bill.credit_limit != null;
  ...
  const {
    ...
    for (const p of payments) {
      ...
      if (p.date_iso) {
        ...
        paymentsByDateMap.set(key, (paymentsByDateMap.get(key) || 0) + Number(p.amount));
      }
    }

    const totalMonthlyBills = total;
    const totalIncome = 1; // TODO: wire real income; avoid divide-by-zero
  ...
  return (
    <main className="min-h-screen bg-zinc-950 text-white">
      <section className="mx-auto max-w-6xl px-6 py-10 space-y-10">
        ...
        {/* Payments over time + due timeline */}
        <div className="grid gap-6 lg:grid-cols-[1.1fr_0.9fr]">
          <div className="rounded-3xl border border-white/10 bg-white/5 p-6">
            <div className="flex items-center justify-between gap-2">
