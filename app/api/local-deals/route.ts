...
export async function POST(request: Request) {
  ...
  try {
    ...
    return NextResponse.json(
      { alternatives: [], message: "Location is required" },
      { status: 400 }
    );
  }
  ...
  try {
    if (query) {
      const response = await fetch(
        "https://places.googleapis.com/v1/places:searchText",
        {
          ...
          headers: {
            ...
            "X-Goog-FieldMask":
              "places.id,places.displayName,places.location,places.googleMapsUri,places.priceLevel",
          },
          body: JSON.stringify({
            textQuery: query,
            maxResultCount: 8,
      ...
      if (!response.ok) {
        ...
        return NextResponse.json({
          alternatives: [],
          message: "Nearby options are temporarily unavailable",
        });
      }

      const data = (await response.json()) as { places?: GooglePlace[] };
      const alternatives = mapPlaces(data.places || [], lat, lng, currentMerchant);

      return NextResponse.json({
        alternatives,
        message: alternatives.length
          ? `Nearby places for “${query}”`
          : `No nearby places found for “${query}”`,
      });
    }
    ...
    return NextResponse.json({
      alternatives,
      message: alternatives.length
        ? "Nearby places that may be worth comparing"
